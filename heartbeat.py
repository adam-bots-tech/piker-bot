...
def pulse():
	try:
		...
		trades_manager.pull_queued_trades(j, trades_db)
		...
		if (is_open == None):
			...
			if trades_db.get_market_open() == True:
				logging.critical('Market has closed.')
				trades_db.set_market_open(False)
			return

		if trades_db.get_market_open() == False:
		...
		trades_manager.open_new_trades(b, sm, j, trades_db)
